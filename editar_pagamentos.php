...
$query = "select * from pag_pagar where id=$id and controle=0";
$sql = mysql_query($query);

echo '<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">';
if($sql){
	$pag_descricao = mysql_result($sql, 0,'pag_descricao');
	$parcelas = mysql_result($sql, 0,'pag_n_parcelas');
	$pag_valor = mysql_result($sql, 0,'pag_valor');
	$status = mysql_result($sql, 0, 'status');
	$data = mysql_result($sql, 0,'data');
	$data_format = formata_data($data);
	$nome_recebedor = mysql_result($sql, 0,'nome_recebedor');
...
function valida_pagamento(){
	var id = <?php echo $id;?>;
	var nome_paciente = document.getElementById("nome_recebedor").value;
	...
	window.location.href = "valida_editar_pagamento.php?id="+id+"&nome_recebedor="+nome_paciente+"&data="+data+"&valor="+valor+"&parcelas="+parcelas+"&descricao="+descricao+"&status="+status;
}
</script>		
						...
						<div class="contact-form">
							...
							<div class="row field_text alignleft ">
								<p align="left">
									<label><strong>Valor R$*</strong> </label><br /> <input
										...
										id="valor" value="<?php echo $pag_valor;?>"
							...
							<div class="row field_textarea alignleft">
								...
								<textarea id="descricao" name="descricao"
									class="textarea textarea_middle required" cols="40" rows="10"><?php echo $pag_descricao;?></textarea>
							</div>
